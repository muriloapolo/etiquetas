document.addEventListener("DOMContentLoaded", () => {
    ...
    btnGeraEtiquetaBtn.addEventListener('click', () => {
        ...
        dadosEtiqueta = {
            cliente: document.getElementById('nomeCliente').value,
            chave: document.getElementById('chaveAcesso').value.replace(/\s+/g, ''), // Remove espaços
            cidade: document.getElementById('nomeCidade').value,
            estado: document.getElementById('estado').value,
            volumes: parseInt(document.getElementById('volumes').value),
            nota: document.getElementById('notaFiscal').value
    ...
    btnConfirmaEtiqueta.addEventListener('click', () => {
        ...
        // Preparar a tela para impressão
        ...
        // Aguarda meio segundo para renderizar as imagens do QR Code e chama a impressora
        setTimeout(() => {
            window.print();
            
            // Restaurar tela ao fechar a janela de impressão
...
class Etiqueta {
    ...
    geraTotalEtiqueta() {
        ...
        // INJEÇÃO DINÂMICA DE TAMANHO DE PÁGINA
        ...
        newArraySlice.forEach((divisores, op) => {
            ...
            divisores.forEach((it, indexCalculado) => {
                let uniqueQrcodeId = `qrcode-${op}-${indexCalculado}`;
                ...
                let cardHtml = `
                    <div class="card cardEtiquetasImp">
                        ...
                        <div class="qrcode-container" style="width: 100%; display: flex; justify-content: center; margin-top: auto; padding-top: 5px;">
                            <div id="${uniqueQrcodeId}"></div>
                        </div>
                    </div>`;
        ...
        newArraySlice.forEach((divisores, op) => {
            divisores.forEach((it, indexCalculado) => {
                ...
                try {
                    new QRCode(document.getElementById(uniqueQrcodeId), {
                        text: textoCodigo,
                        width: 55, // Tamanho otimizado para a altura de 54mm do seu layout
                        height: 55,
                        colorDark: "#000000",
                        colorLight: "#ffffff",
                        correctLevel: QRCode.CorrectLevel.L
